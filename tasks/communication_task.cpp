...
namespace tasks {
    communication_task::communication_task(osMessageQueueId_t ambientTemperatures,
                                           ...
                                           osMessageQueueId_t internalHumidity,
    ...
    void communication_task::run() {
        ...
        while (true) {
            ...
            if (osMessageQueueGetCount(_internalHumidity) > 0) {
                std::int32_t tempValue;
                osMessageQueueGet(_internalHumidity, &tempValue, nullptr, 0);
                if (tempValue > 0) {
                    _last_internal_humidity = tempValue;
                }
            }
            ...
            if (auto currentTick = osKernelGetTickCount(); currentTick > lastTick + 15000) {
                ...
                payload.append(std::to_string(_last_internal_humidity).data());
                payload.append(R"(})");
                ...
                if (uartRxData[0] == 0xAA && uartRxData[1] == 0xAA) {
                    ...
                    for (const auto &parameterString: parameters) {
                        ...
                        if (parameter == "TARGET_TEMP") {
                            ...
                            responseStr.append("HUMIDITY IS NOT SUPPORTED CURRENTLY!");
                            continue;
                            ...
                            responseStr.append(parameter);
                            responseStr.append(" IS NOT SUPPORTED\r\n");
                            continue;
                        }
                        responseStr.append(parameter);
                        responseStr.append(" SET!\r\n");
                    }
                    responseStr.append("\x03");
                    HAL_UART_Transmit_DMA(&huart1, reinterpret_cast<uint8_t *>(responseStr.data()), responseStr.size());
                    osDelay(1);
                }
            }
            uartRxData.fill(0);
    ...
    void communication_task::handle_command(std::uint8_t command) {
        switch (static_cast<types::command>(command)) {
            ...
            case types::command::DISABLE_COOLING: {
                ...
                etl::string<20> responseStr{"Toggled Cooling!\r\n"};
                HAL_UART_Transmit(&huart1, reinterpret_cast<std::uint8_t *>(responseStr.data()),
                                  responseStr.size(), 100);
                break;
            }
    ...
    void communication_task::connect_wifi() {
        etl::string<32> response{};
        if (!_settings.wifiSsid.empty() && !_settings.wifiPassword.empty()) {
            _mqtt.connect_to_wifi(_settings.wifiSsid, _settings.wifiPassword);
        } else {
            response.append("WIFI PARAM MISSING:");
            response.append(_settings.wifiSsid.empty() ? "0" : "1");
            response.append(_settings.wifiPassword.empty() ? "0" : "1");
            response.append("\r\n");
        }

        response.append("\r\n");
        HAL_UART_Transmit_DMA(&huart1, reinterpret_cast<std::uint8_t *>(response.data()), response.size());
    }

    void communication_task::connect_mqtt() {
        etl::string<32> response{};
        if (_mqtt.is_wifi_connected()) {
            if (!_settings.mqttBroker.empty() && !_settings.mqttUsername.empty() && !_settings.mqttPassword.empty() && !
                _settings.deviceCode.empty()) {
                _mqtt.clean_mqtt();
                osDelay(100);
                ...
                // TODO: Include last will in connection package.
